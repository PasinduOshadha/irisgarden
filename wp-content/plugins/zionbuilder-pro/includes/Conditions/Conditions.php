...
class Conditions {
	...
	public function __construct() {
		add_action( 'admin_enqueue_scripts', [ $this, 'on_enqueue_scripts' ], 1 );
		add_action( 'zionbuilder/editor/before_scripts', [ $this, 'on_enqueue_scripts' ], 1 );
	...
	/**
	 * Will register the needed scripts for the conditions components
	 *
	 * @return void
	 */
	public function on_enqueue_scripts() {
		wp_register_style(
			'zb-pro-conditions',
			Utils::get_file_url( 'dist/css/conditions.css' ),
		...
		wp_register_script(
			'zb-pro-conditions',
			Utils::get_file_url( 'dist/js/conditions.js' ),
			[
				'zb-components',
			],
			Plugin::instance()->get_version(),
		...
		wp_localize_script(
			...
			'ZnPb_ConditionsData',
			[
				'conditions'            => $this->get_conditions_for_admin(),
				'categories'            => $this->get_categories(),
				'conditions_saved_data' => $this->get_conditions_saved_data(),
			]
		);
	...
	public function register_default_conditions() {
		...
		$excluded_post_types = apply_filters( 'zionbuilderpro/conditions/exluded_post_types', [ ZBFreeTemplates::TEMPLATE_POST_TYPE ] );
	...
	public function get_condition_options( $condition_id, $arguments = [], $search_keyword = '', $page = 1 ) {
		$condition = $this->get_condition( $condition_id );
