...
use ZionBuilder\Utils as ZionBuilderUtils;
...
class ThemeBuilder {
	...
	public function __construct() {
		...
		add_action( 'zionbuilder/frontend/init', [ $this, 'on_zion_frontent_init' ] );
	...
	function register_template_types( $template_manager ) {
		// Register headers
	...
	public function enqueue_scripts( $hook ) {
		if ( $this->admin_page_hook === $hook ) {
			...
			wp_enqueue_style(
				...
				Utils::get_file_url( 'dist/css/theme-builder.css' ),
				[ 'zb-components', 'zb-pro-conditions' ],
			...
			if ( is_rtl() ) {
				wp_enqueue_style(
					...
					Utils::get_file_url( 'dist/css/rtl-pro.css' ),
					[],
					Plugin::instance()->get_version()
				);
			};
			...
			wp_enqueue_script(
				...
				Utils::get_file_url( 'dist/js/theme-builder.js' ),
				[
					'zb-components',
					'zb-pro-conditions',
				],
			...
			wp_localize_script(
				...
				[
					...
					'urls'           => [
						'logo_url' => ZionBuilderUtils::get_logo_url(),
					],
				]
			);
		}
	 ...
	 */
	public function on_zion_frontent_init( $frontend ) {
	...
	public function get_assigned_template() {
		...
		foreach ( $templates as $template_config ) {
			...
			if ( $should_apply ) {
				foreach ( $template_config['template_config'] as $area_id => $area_config ) {
					$this->extract_area( $area_id, $area_config );
				}
				return;
			}
	...
	public function extract_area( $area_id, $area_config ) {
		if ( isset( $area_config['content'] ) && isset( $area_config['active'] ) && $area_config['active'] ) {
			...
			$this->registered_templates[$area_id] = $post_id;
		}
	}
	...
	public function on_template_include( $template ) {
		...
		// Check to see if the body is overwriten. It will be disabled in preview mode
		if ( $body_override ) {
			return dirname( __FILE__ ) . '/views/body.php';
		}
	...
	public function override_template( $type, $name ) {
		// Add our override
		require dirname( __FILE__ ) . "/views/{$type}.php";
