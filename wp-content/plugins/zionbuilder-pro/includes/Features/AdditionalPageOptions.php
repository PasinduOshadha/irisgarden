...
class AdditionalPageOptions {
	...
	public function __construct() {
		// Filters
		add_filter( 'zionbuilder/page_assets/css', [ $this, 'add_cutom_css' ], 10, 2 );
		add_filter( 'zionbuilder/page_assets/js', [ $this, 'add_custom_javascript_to_page' ], 10, 2 );
	...
	public function add_cutom_css( $css, $page_assets_instance ) {
		$page_ids = $page_assets_instance->get_post_ids();
	...
	public function add_custom_javascript_to_page($js, $page_assets_instance) {
		$page_ids = $page_assets_instance->get_post_ids();
		if (is_array( $page_ids )) {
			foreach ($page_ids as $post_id) {
				// Add post custom js
