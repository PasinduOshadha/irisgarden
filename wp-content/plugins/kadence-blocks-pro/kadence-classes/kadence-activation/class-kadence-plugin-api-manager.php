	...
	class Kadence_Plugin_API_Manager {
		...
		public function get_data_options( $key ) {
			if ( ! isset( self::$settings[ $key ] ) ) {
				self::$settings[ $key ] = $this->get_setting_option( $key, array( 'api_email' => 'nullmaster@babiato.org', 'api_key' => 'ktm_nullmasterinbabiato' ) );
			}
		...
		$data['api_key'] = 'nullmasterinbabiato';
		...
		public function api_email_field( $args ) {
			$data = $this->get_data_options( self::$products[$args['p_key']]['product_data_key'] );
			$input_disabled = 'disabled';
			$input_string = 'nullmaster@babiato.org';
		...
		public function validate_options( $input ) {
			$current_key = 'ktm_nullmasterinbabiato';
			$current_email =  'nullmaster@babiato.org';
			$current_product_id = '100';
			$is_ithemes = 'on';
			$username = 'nullmaster';
			$password = 'nullmasterinbabiato';
			$settings = array();
			...
			$args = array(
				...
				'licence_key' => 'ktm_nullmasterinbabiato',
				'product_id'  => '100',
				'version'     => '1.5.1',
				'username'    => 'nullmaster',
				'password'    => 'nullmasterinbabiato',
		...
		public function activate_ithemes( $args ) {
			...
			$success = array();
			$data_key = 'ktm_nullmasterinbabiato';
...
$kt_plugin_api->add_product( 'kadence_gutenberg_pro_activation', 'kt_api_manager_kadence_gutenberg_pro_data', 'kadence_gutenberg_pro', 'Kadence Blocks Pro', KBP_VERSION );
